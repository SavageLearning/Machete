...
field_names = ('sid', 'name', 'address1', 'address2', 'city', 'state', 'zipcode',
               'business', 'phone', 'cellphone', 'email', 'referredby',
               'active', 'blogparticipate', 'notes', 'referredbyOther')
...
for send_data in all_submissions:
    send_data['ID'] = '0'
    try:
        post_response = s.post(url='http://192.168.2.109/Employer/Create', data=send_data)
    except:
        print "!````````````` Update failed (MAIL SEND)"
#        mail("Machete post failed", "Posting the form to the Machete URL failed")

    print "--------------- post_response: ", post_response.text

    cursor.execute("SELECT sid FROM webform_machete WHERE sid='%s'", send_data['sid'])
    existing_entry = cursor.rowcount

    print "send_data[sid]:", send_data['sid']
    print "existing_entry: ", existing_entry
    if (not 'an error occurred' in post_response.text and post_response.json['jobSuccess']):
        print "JobSuccess!"
        if (existing_entry > 0):
            try:
                cursor.execute("UPDATE webform_machete SET success='1', tries=tries+1, last_attempt=NOW() WHERE sid='%s'", (send_data['sid'],))
                db.commit()
            except:
                print "!````````````` Update webform_machete table failed"
#                mail("update DB failed", "Updating webform_machete table failed")
        else:
            try:
                cursor.execute("INSERT INTO webform_machete (sid, success, tries, last_attempt) VALUES('%s', '%s', 1, NOW())",(send_data['sid'], post_response.json['jobSuccess']))
                db.commit()
            except:
                print "!````````````` Insert into webform_machete table failed"
#                mail("Insert DB failed", "Inserting into webform_machete table failed")
    else: #jobSuccess failed
        print "JobFailed!"
        if (existing_entry > 0):
            print "blip 1"
            try:
                cursor.execute("UPDATE webform_machete SET success=0, tries=tries+1, last_attempt=NOW() WHERE sid='%s'", (send_data['sid'],))
                db.commit()
            except:
                print "````````````` Update webform_machete table failed"
#                mail("update DB failed", "Updating webform_machete table failed")
            ...
            print "blip 2"
            try:
                cursor.execute("INSERT INTO webform_machete (sid, success, tries, last_attempt) VALUES('%s', 0, 1, NOW())", (send_data['sid'],))
                db.commit()
            except:
                print "!````````````` Insert into webform_machete table failed"
#                mail("Insert DB failed", "Inserting into webform_machete table failed")
