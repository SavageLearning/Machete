    ...
    public class WorkerSigninService : IWorkerSigninService
    {
        ...
        public ServiceIndexView<WorkerSigninView> GetIndexView(DispatchOptions o)
        {
            ...
            if (!string.IsNullOrEmpty(o.search))
            {
                ...
                //                p.worker.Person.firstname1.Contains(o.search) ||
                //                p.worker.Person.firstname2.Contains(o.search) ||
                //                p.worker.Person.lastname1.Contains(o.search) ||
                //                p.worker.Person.lastname2.Contains(o.search)).ToList();      
            }
            else
            {
                enumWSI = queryableWSI.ToList();
            ...
            //enumWSIV = enumWSI
            //            .GroupJoin(WorkerCache.getCache(), s => s.dwccardnum, w => w.dwccardnum, (s, w) => new { s, w })
            //            .SelectMany(jj => jj.w.DefaultIfEmpty(new Worker { ID=0, dwccardnum = 0}), (jj, row) 
            //             => new WorkerSigninView(row.Person, jj.s));
